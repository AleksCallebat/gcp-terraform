...
#  NOTE : ON DELETION, THIS NEEDS TO BE DELETED FROM THE STATE FIRSTH WITH `terraform state rm databricks_metastore_data_access.first`

# ADD WORKSPACE TO THE METASTORE
resource "databricks_metastore_assignment" "this" {
  depends_on = [ module.gcp-workspace-full,databricks_metastore.new_metastore ]
  provider             = databricks.accounts
